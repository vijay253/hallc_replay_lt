...
void calibration::Terminate()
{
  ...
  for (Int_t ipmt=0; ipmt < (fhgc_pmts); ipmt++)
      ...
      if (!fTrack)
	{
	  ...
	  fscaled[ipmt] = new TH1F(Form("fscaled_PMT%d", ipmt+1), Form("Scaled ADC spectra for PMT%d; NPE; Normalized Counts",ipmt+1), 100, 0, 10);
	  ...
	  Poisson->SetParameter(0, 5.0);
	  Poisson->SetParameter(1, 0.1);
	  ...
	  if (fFullShow) fscaled_nobackground[ipmt]->GetYaxis()->SetRangeUser(0,0.7);
	  ...
	  fscaled_mk2[ipmt] = new TH1F(Form("fhgc_scaled_mk2_PMT%d", ipmt+1), Form("Scaled ADC spectra for PMT%d; NPE; Normalized Counts",ipmt+1), 200, 0, 20.0);
